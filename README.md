...
> 非技術者の施設担当者向け。**詳細な配布・展開手順は IT 担当者が実施**し、本セクションは日常運用の入り口として参照してください。

### インストール

配布 ZIP（`wiseman-hub-vX.Y.Z-win-x64.zip`）を USB または社内配信で受領したら、IT 担当者に依頼して展開してもらってください。詳細手順は **[14c-deploy.md](docs/handoff/14c-deploy.md)** を参照。

展開後、デスクトップに「**Wiseman PDF ツール**」のアイコンが作成されます。
