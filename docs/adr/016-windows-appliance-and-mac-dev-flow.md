...
| **runtime critical path** (PR-4 後) | 上記 + `updater.py`（download/spawn/rollback/lock/preflight） | 800 LOC |
...
PR-4 codex review (threadId 019dfd43) 反映後の実測値: **782 LOC** (pygount summary、PR-3 から +393 LOC):

- C-1 (returncode==0 早期終了の rollback 抑制) / C-2 (多重起動 lock + stale 解除) /
  C-4 (preflight) / I-1 (artifact size cap, Content-Length 事前 + chunked 累計) /
  I-2 (Windows 互換 mkstemp + os.replace) / I-3 (rollback spawn の同 timeout 監視) /
  I-5 (timeout test injection) を全反映した結果
- 当初 300 → 400 → 700 → 800 LOC へと段階緩和したが、800 LOC 超過時はアーキテクチャ見直し
  （updater 分割、Sigstore 検証 module 別 PR 化等）を必須とする最終ラインとする

緩和の根拠:

- launcher 極小性の本質は「**stdlib only** + **`wiseman_hub.*` import 禁止** +
  **依存ライブラリ重量ゼロ**」であり、行数自体ではない
- 行数を盲目的に維持するために validation / 排他制御 / preflight を削るのは
  supply-chain 防御 / 業務継続性の毀損
- 800 LOC は PR-6 (provenance verify 本実装) で +30〜50 LOC を見込む余裕も含む
- それでも 800 LOC を超える場合は updater module 分割を検討する

行数監視:

- 各 PR で `uvx pygount src/wiseman_hub_launcher --format=summary` の値を PR body に記載
- 800 LOC 超過時は「launcher が肥大化している signal」としてアーキテクチャ見直し
...
- `--update` (本番 update mode) は default で fail-closed（PR-6 で provenance 実装後に解除）
- テスト用途のみ `--allow-insecure-checksum-only` 明示フラグで bypass 可能
- `--dry-run` / `--no-spawn` 系は `--allow-insecure-checksum-only` 不要（download/spawn しないため）
