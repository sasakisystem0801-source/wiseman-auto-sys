...
| **各 module 単体** | 上記すべて | **270 LOC** (実測最大 262 = `__main__.py`) |
...
415 LOC 制約を突破するため、**事前に `_supply_chain/sigstore.py` を切り出す**:
...
これにより `_supply_chain/` の 415 LOC 制約を維持しつつ、PR-6 後半で
...
| **PR-6a (本 PR)** | SHA-256 + SLSA v1.0 statement claims (subject digest + name + multi-subject 一意性 + predicateType + workflow ref + repo + builder id allowlist) | `--allow-test-unsigned-provenance` flag + 環境変数 `WISEMAN_ALLOW_UNSIGNED_PROVENANCE_FOR_TESTS=1` の **AND 二重 gate**、本番 PC では env 不在で必ず fail-close |
| **PR-6 後半** | 上記 + Sigstore Bundle 署名検証 (sigstore-python 統合) | flag 完全除去、本格 fail-closed |
...
| dependency | 必要な理由 |
|-----------|-----------|
| **PR-6 後半マージ済 (sigstore-python 統合 + signature 検証本実装)** | claims verify (PR-6a) のみでは「攻撃者が release-prod bucket に書ける前提」で偽造 provenance を通せる。SLSA 元来の脅威モデル (sigstore で証明される build provenance) を満たすには signature 検証が必須 |
| **`--allow-test-unsigned-provenance` flag が launcher CLI から削除済** | PR-6 後半で完全 fail-closed (二重 gate も除去)、canary 検証で削除確認後のみ Phase 7 着手可 |
