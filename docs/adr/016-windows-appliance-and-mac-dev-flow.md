...
| **runtime critical path** (PR-4 後) | 上記 + `updater.py`（download/spawn/rollback/lock/preflight/heartbeat） | 900 LOC |
| **対象外** | `__init__.py`（version 文字列のみ）、test files、PyInstaller spec | 制約なし |

PR-3 codex review (threadId 019dfce6) 反映後の実測値: **389 LOC**（path traversal 防御強化、HTTPS pin、DoS cap、provenance signature 拡張等で +103 LOC）。

PR-4 codex review (threadId 019dfd43、計画段階) 反映後の中間実測値: **782 LOC** (PR-3 から +393 LOC、C-1/C-2/C-4/I-1/I-2/I-3/I-5 全反映)。

PR-4 codex second-pass review (threadId 019dfd5d、PR 段階) 反映後の最終実測値: **842 LOC** (pygount summary、+60 LOC):

- Critical 1 (lock heartbeat、long-running download での自己 stale 化防止): +30 LOC
- Important 1 (`current_path` 引数で canary/test override 整合性): +5 LOC
- Important 2 (artifact download の redirect 後 URL 検証): +5 LOC
- Important 3 (mkdir/mkstemp も try 内に正規化): +5 LOC
- Important 4 (artifact replace 後の dir fsync): +5 LOC
- Suggestion 1 / 2 (stale unlink race / `--monitor-timeout` 正値制約): +10 LOC

900 LOC への最終調整理由 (300 → 400 → 700 → 800 → 900):

- 計画段階 + PR 段階の 2 回 codex review で「validation を削って LOC 維持は supply-chain
  防御毀損」と明示されたため、防御装備を完備したうえでの実測値に上限を合わせる
- 900 LOC は PR-6 (provenance verify 本実装) で +30〜50 LOC を見込む余裕も含む最終ライン
- これ以上拡大する場合は updater 分割 (例: `_lock.py` / `_download.py` 切り出し) を必須化

行数監視:

- 各 PR で `uvx pygount src/wiseman_hub_launcher --format=summary` の値を PR body に記載
- 900 LOC 超過時は「launcher が肥大化している signal」として module 分割を強制 (再緩和不可)
...
##### 2.1 PR-4 成果物の本番配布禁止ゲート（PR-4 codex review 反映）

PR-4 codex review (threadId 019dfd43, C-3 反映) で確定:

PR-4 単体は SHA-256 のみ検証で provenance 未呼び出しのため、canary / 業務責任者 PC への
配布は **絶対に禁止**。PR-6 (release workflow + provenance 検証 + GitHub OIDC) マージ後に
canary 解禁。実装上の安全装置:

- `--update` / `--update --no-spawn` のいずれも **`--allow-insecure-checksum-only` 必須**
  (download + current.json 切替を行うため、PR-6 で provenance 実装後に解除)
- `--dry-run` のみ `--allow-insecure-checksum-only` 不要 (read-only、副作用ゼロ)
- テスト用途のみ `--allow-insecure-checksum-only` 明示フラグで bypass 可能

これにより:

- PR-4 マージ後も Mac dev 環境 / smoke build / unit test では `--allow-insecure-checksum-only` で動作確認可能
- Windows 実機 / 本番 manifest を指定した `--update` は PR-6 マージまで実行不可
- `--allow-insecure-checksum-only` は migration audit が grep 容易なフラグ名（PR-6 で除去）
...
#### Phase 7 切替の hard dependency（PR-4 codex review 反映）

PR-4 codex review (threadId 019dfd43, C-3) で確定:

| dependency | 必要な理由 |
|-----------|-----------|
| **PR-6 マージ済** | provenance 検証本実装。これなしで PR-4 を本番配布すると SHA-256 単独防御となり真正性ベース供給網防御（§1.1.1）が成立しない |
| **PR-5 runbook の seed 手順反映済** | 初回配置時の `versions/{初期版}/wiseman_hub.exe` seed なしでは preflight 失敗で起動不能（§2.2） |
| **launcher の本番 PC 配置完了** | wiseman_launcher.exe は固定配布（§2 bootstrapper 構成）、初回のみ手動配布 |

PR-4 は dev 検証 / smoke build / unit test 専用とし、本番 canary 切替は上記 3 件揃った時点で解禁。
