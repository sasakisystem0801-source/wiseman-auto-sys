...
### 確定方針: Phase 2b 完了 → 即座に exe 配布実行

Session 75 終了時にユーザーと合意した配布タイミング:
- 本セッション (Phase 2a 完了) 単独では配布しない (内部リファクタのみで実機差なし)
- **Phase 2b 完了後** に exe 配布 (Phase 3 は別配布タイミングへ)
- 理由: Phase 2b は `facility_root_dialog.py` を触るため UI 経路の実機検証と一緒にできる + 配布回数を最小化 (本田様の作業負担軽減)
- Phase 3 (UNC パス含む) は Windows-only 検証が必要なため別配布タイミングが安全

### 次セッション実行手順

1. **Issue #27 続編 G Phase 2b 実装** (`facility_root_dir`、157 consumer)
   - Phase 2a パターン踏襲: `_check_path` 1 件追加、load_config の `coerce_path` 適用、`ui/facility_root_dialog.py` の `Path()` 重複ラップ除去
   - **Windows CI 早期確認推奨** (debt #1)、`str(Path("..."))` パターン徹底
   - Quality Gate 4 段 (`/simplify` + `/safe-refactor` + `evaluator` + `/codex review`)
   - PR 作成 → CI 全 PASS → ユーザー承認 → merge

2. **exe 配布 7 ステップ** (Phase 2b PR merge 直後、`docs/handoff/1c-exe-redistribution-runbook.md` 準拠)
   - 1: `git pull --ff-only` (Windows 機、本田様 PC)
   - 2: 現行 exe を `.bak-<stamp>` でバックアップ
   - 3: `uv sync --extra dev && uv run pytest -q -m "not integration"`
   - 4: `uv run pyinstaller wiseman_hub.spec --clean --noconfirm`
   - 5: build.log warning 検査
   - 6: `Copy-Item -Force dist\wiseman_hub.exe $HOME\wiseman-hub\`
   - 7: Launcher 起動 + 実機検証 7 件 (#274 / #282 / Launcher 5 ボタン / log_level / trust root WARNING / Path 移行 Phase 1 / Path 移行 Phase 2a + 2b)
   - 所要時間: 30-45 分。手順 1-6 は AI 伴走 (TeamViewer 経由 PowerShell)、手順 7 は本田様の確認も含む

3. **配布完了後の handoff** (Session 76 終了時)
   - 実機検証結果を LATEST.md に記録
   - 残課題: Phase 3 (UNC パス、`Optional[Path]` 設計議論)、Issue #275
