...
手順（詳細は Session 20 成果セクション「1-C 完走のための残作業」参照）:
1. TeamViewer で Windows PC 接続、`git pull` + `uv sync`
2. `uv run pyinstaller wiseman_hub.spec --clean --noconfirm`
3. `dist\wiseman_hub.exe` を `%USERPROFILE%\wiseman-hub\wiseman_hub.exe` に上書き
4. Launcher 4 ボタン目「事業所フォルダ結合」動作確認、Session 19 と同じ 19 件結合の再現
5. 問題なければ ADR-011 Accepted 昇格（14D 兼務）
