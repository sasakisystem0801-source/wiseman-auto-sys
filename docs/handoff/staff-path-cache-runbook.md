...
## Phase 2: 翌月以降の運用（毎月 cache populate が必要）

⚠ **重要**: cache key は `"{staff}:{year}:{month}"` 形式のため、3 月分を記憶しても
4 月分は別キーとして扱われ cache miss する（実装通り、コード側 cache_key 関数参照）。
**月別 xlsx ファイルが切り替わる業務である以上、毎月 5 担当者ぶんの初回 populate が必要**。

毎月の運用フロー:
1. 業務責任者が新月分の xlsx を担当者フォルダに用意
2. C ダイアログで対象月を新しい月に切り替え → 「対象行を読込」
3. 担当者ごとに `▶ 要レビュー` → ダブルクリックで xlsx 選択
4. **「この選択を記憶」を ON** にして OK（次回以降同じ月の再 plan 時は cache hit）
5. 5 担当者分繰り返し

cache miss が出るのは:
- **対象月の初回 plan**（毎月発生、上記の手順で対応）
- **新月分のファイルがまだ無い**（業務の進捗待ち、`SKIPPED_NO_XLSX` か候補ゼロ NEEDS_REVIEW）
- **命名規則が変わった**（→ 業務責任者へ確認、ADR-015 前提崩壊）
- **担当者が増えた**（→ TOML の `[checklist.report_staff."<name>"]` に新 entry 追加）
