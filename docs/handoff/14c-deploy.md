...
1. **タスクマネージャで `wiseman_hub.exe` プロセスを確認**
   - プロセスが残っている（応答なし） → Tkinter 以外の初期化エラーの可能性。§7.2.a のダンプ取得へ
   - プロセスが見当たらない（起動直後に消える） → §7.2.b のログ取得へ

2. **個別エラー別の対処**
   - Tkinter ランタイムエラー → `14a-build.md` の「原因候補」を参照
   - `config\default.toml` が未配置 → §2.3 を再確認し `default.toml.sample` からコピー
