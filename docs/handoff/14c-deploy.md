...
2. テキストエディタ（メモ帳可）で `config\default.toml` を開き、`■ 必須` マーカーの付いた行を施設固有の設定に編集:
   - `[wiseman]` `exe_path`（ワイズマンのショートカット / exe パス）
   - `[gcp]` `service_account_key_path`（受領した `sa-key.json` の配置先）
   - `[ocr_backend]` `endpoint_url` / `api_key`（弊社から別途受領）
   - `[pdf_merge]` `input_dir` / `output_dir` / `source_a_filename`
   - `[pdf_merge.user_name_bbox]` `x0` / `y0` / `x1` / `y1`（A の氏名切り出し矩形、施設別）
...
1. **タスクマネージャで `wiseman_hub.exe` プロセスを確認**
   - プロセスが残っている（応答なし） → §7.2.a のダンプ取得へ
   - プロセスが見当たらない（起動直後に消える） → §7.2.b のログ取得へ

2. **ログ（`startup.log`）確認後の個別対処**（§7.2.b で取得したログをもとに判断）
   - ログに Tkinter 関連 Traceback → `14a-build.md` の「原因候補」を参照
   - ログに TOML 読込エラー → `config\default.toml` 未配置。§2.3 を再確認し `default.toml.sample` からコピー

#### 7.2.a プロセス残存（応答停止）時のダンプ取得

タスクマネージャで `wiseman_hub.exe` を右クリック → 「ダンプ ファイルの作成」。
生成された `.dmp` ファイルの保存先（通常 `%LOCALAPPDATA%\Temp\wiseman_hub.DMP`）を弊社担当へ連絡して共有してください。

#### 7.2.b 起動直後に終了する場合のログ取得（推奨手順）

GUI exe は標準出力が表示されないため、コマンドラインから起動してログをファイルに保存します。

**PowerShell の場合**（Windows 11 既定）:

```powershell
cd $env:USERPROFILE\wiseman-hub
.\wiseman_hub.exe *> startup.log
# 終了（または自動終了）後:
notepad startup.log
```
...
```cmd
cd %USERPROFILE%\wiseman-hub
wiseman_hub.exe > startup.log 2>&1
notepad startup.log
```

- `*>`（PowerShell）/ `2>&1`（cmd）: 標準エラー出力も含めて取得
- 保存先: `%USERPROFILE%\wiseman-hub\startup.log`
- ログに `Traceback` / `Error` / `FATAL` の文字列があれば弊社担当へ連絡し、`startup.log` を添付可能
- **`startup.log` が空 or 0 バイトの場合**: Python 起動前の Windows 側クラッシュ（`Fatal Python error` や ntdll 経由）の可能性があり、stdout/stderr に出力されない。§7.2.a のダンプ取得を優先してください。

**連絡前の個人情報確認**: `startup.log` にフォルダパス（例: 利用者氏名を含む `output_dir`）が含まれる場合があります。
連絡前に開いて氏名・電話番号等が記載されていないか目視確認してください。必要に応じて該当行を削除してから送付します。
