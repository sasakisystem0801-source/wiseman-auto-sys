...
""",
        ...
        cfg.pdf_merge = replace(cfg.pdf_merge, input_dir="/new/in")
        save_config(cfg, target)
    ...
    def test_save_preserves_bbox_nested_section(self, tmp_path: Path) -> None:
        ...
        cfg = AppConfig()
        # Issue #27 続編 E Phase 1/2: UserNameBBox / PdfMergeConfig 双方 frozen=True。
        cfg.pdf_merge = replace(
            cfg.pdf_merge,
    ...
    def test_save_with_inline_table_notation(self, tmp_path: Path) -> None:
        ...
        cfg.wiseman = replace(cfg.wiseman, startup_wait_sec=25)
...
""",
    ...
    def test_save_facility_root_dir_roundtrip(self, tmp_path: Path) -> None:
        ...
        cfg = AppConfig()
        cfg.pdf_merge = replace(cfg.pdf_merge, facility_root_dir="//Tera-station/share/03.FAX(事業所)")
...
""",
        ...
        cfg = load_config(target)
        cfg.pdf_merge = replace(cfg.pdf_merge, facility_root_dir="/srv/facility")
    ...
    def test_save_concat_order_reorder(self, tmp_path: Path) -> None:
        ...
        cfg = load_config(target)
        cfg.pdf_merge = replace(cfg.pdf_merge, concat_order=("C", "A", "B"))
...
""",
    ...
    def test_save_ex_source_dir_roundtrip(self, tmp_path: Path) -> None:
        ...
        cfg = AppConfig()
        cfg.pdf_merge = replace(cfg.pdf_merge, ex_source_dir="C:\\Users\\sasak\\OneDrive\\デスクトップ\\本田様")
...
""",
        ...
        cfg = load_config(target)
        cfg.pdf_merge = replace(cfg.pdf_merge, ex_source_dir="/srv/ex_source")
...
""",
        ...
        cfg = load_config(target)
        cfg.pdf_merge = replace(cfg.pdf_merge, facility_aliases={"本田デイケア": ["本田DC"]})
...
""",
        ...
        cfg.pdf_merge = replace(cfg.pdf_merge, facility_aliases={})
        save_config(cfg, target)
...
""",
    ...
    def test_save_cleanup_warning_does_not_leak_path_or_pii(
        ...
        cfg = AppConfig()
        cfg.pdf_merge = replace(cfg.pdf_merge, input_dir="/private/施設A/patient/山田太郎")
