...
class MessageBoxLike(Protocol):
    ...
    def askyesno(self, title: str, message: str) -> bool: ...  # noqa: D401

    def showinfo(self, title: str, message: str) -> None: ...  # noqa: D401
...
# Treeview に表示する候補（= まだ人間判断が必要な状態）
_OPEN_STATUSES = frozenset({PairStatus.NEEDS_CONFIRMATION, PairStatus.NO_MATCH})
...
class ConfirmDialog:
    ...
    def __init__(
        ...
        *,
        ...
        if not any(c.status in _OPEN_STATUSES for c in session.candidates):
            raise ValueError(
        ...
        self._closed_all_resolved = False
    ...
    def run(self) -> ConfirmDialogResult:
        ...
        return ConfirmDialogResult(
            resolved_all=self._closed_all_resolved,
    ...
    def _set_buttons_for(self, cand: UserCandidate | None) -> None:
        ...
        has_similar_bc = (
            _pick_first_b(cand.similar_candidates) is not None
            or _pick_first_c(cand.similar_candidates) is not None
        )
    ...
    def _on_reject(self) -> None:
        cand = self._selected_candidate()
        if cand is None or cand.status not in _OPEN_STATUSES:
            return
        self._apply_update(
            cand.page_index,
    ...
    def _check_all_resolved(self) -> None:
        ...
        self._closed_all_resolved = True
        self._root.quit()
    ...
    def _progress_text(self) -> str:
        ...
        resolved = sum(1 for c in self._session.candidates if c.is_resolved)
        open_count = total - resolved
        needs_conf = sum(
            1 for c in self._session.candidates if c.status == PairStatus.NEEDS_CONFIRMATION
        )
        no_match = sum(
            1 for c in self._session.candidates if c.status == PairStatus.NO_MATCH
        )
        return (
...
def compute_approve_decision(
    ...
    if cand.status != PairStatus.NEEDS_CONFIRMATION:
        return None
    first_b = _pick_first_b(cand.similar_candidates)
    first_c = _pick_first_c(cand.similar_candidates)
    if first_b is None and first_c is None:
        return None
    return (first_b, first_c)


def _pick_first_b(similar: list[CandidateState]) -> str | None:
    for c in similar:
        if c.kind == "B":
            return c.path
    return None
...
def _pick_first_c(similar: list[CandidateState]) -> str | None:
    for c in similar:
        if c.kind == "C":
            return c.path
    return None
