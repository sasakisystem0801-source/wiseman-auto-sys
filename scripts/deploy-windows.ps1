...
.NOTES
    対応シェル:
        ...
        - Linux / macOS pwsh は非対応 (Tera-station NAS / Get-Process 等が無効)

    実行ポリシー制限時:
        Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process

    エンコーディング: UTF-8 BOM 付き（PS 5.1 で日本語メッセージが CP932 誤解釈
    されないよう BOM を残すこと）。
#>
...
$head = (cmd /c "git rev-parse --short HEAD").Trim()
Write-OK "HEAD: $head"
...
Write-Host "    Get-ChildItem `"$DIST_EXE.bak-*`" | Sort LastWriteTime -Desc | Select -Skip 3 | Remove-Item"
Write-Host ""
